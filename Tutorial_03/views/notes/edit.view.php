...
<main>
    ...
    <form method="POST" action="/notes" class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <textarea id="note" name="note" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"><?= $note['note'] ?? '' ?></textarea>
        <p class="text-red-500 text-sm mt-2"><?= $errors['note'] ?? '' ?></p>
        <div class="mt-6 flex items-center justify-end gap-x-4">
            <a href="/notes" class="rounded-md bg-slate-700 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-slate-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-700 cursor-pointer">Cancel</a>
